...
from flightEnv import AircraftAgentSet, ConflictScene
from flightSim.load import load_data_set, load_and_split_data
from flightSim.model import Routing, FlightPlan
from flightSim.utils import pnpoly
from flightSim.visual import make_random_color
...
def search_routing_in_wuhan(visual=False):
    ...
    if visual:
        ...
        color = make_random_color()
        for [route, idx, *_] in route_list:
            wpt_list = route.waypointList
            idx = [max(idx[0]-1, 0)] + idx[:] + [min(idx[-1]+1, len(wpt_list)-1)]
            points = []
