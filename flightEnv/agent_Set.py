...
from flightSim.utils import make_bbox, distance, build_rt_index
...
class AircraftAgentSet:
    def __init__(self, fpl_list=None, start=None, supply=None, other=None):
        if fpl_list:
            ...
            self.agent_en = other.agent_en[:]
            self.agent_en_ = other.agent_en_[:]
    ...
    def do_step(self, duration=1, basic=False):
        ...
        now += duration
        if now in self.tracks.keys():
            ac_en = self.tracks[now]
        else:
            ac_en = []
    ...
    def detect_conflict_list(self, search=None):
        ...
        check_list = []
        for [a0, *state_0] in self.agent_en[:2]:
