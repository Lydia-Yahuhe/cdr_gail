...
def nature_cnn(unscaled_images, **conv_kwargs):
    ...
    """
    scaled_images = tf.cast(255. - unscaled_images, tf.float32) / 255.

    activ = tf.nn.relu
    x = activ(conv(scaled_images, 'c1', nf=4, rf=9, stride=2, init_scale=np.sqrt(2), **conv_kwargs))
    x = activ(conv(x, 'c2', nf=8, rf=7, stride=2, init_scale=np.sqrt(2), **conv_kwargs))
    x = activ(conv(x, 'c3', nf=16, rf=5, stride=2, init_scale=np.sqrt(2), **conv_kwargs))
    x = activ(conv(x, 'c4', nf=32, rf=5, stride=1, init_scale=np.sqrt(2), **conv_kwargs))
    x = conv_to_fc(x)
